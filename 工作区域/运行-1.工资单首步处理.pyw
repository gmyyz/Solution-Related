...
def _generate_voucher_files(base_dir, company_output_paths, bank_records, payroll_year, payroll_month, log, timesheet_context, bonus_context):
    ...
    for company, company_output_path in company_output_paths.items():
        ...
        actual_voucher_rows = []
        actual_voucher_rows.extend(_build_a1_rows(company, company_code, company_ws, total_row_idx, bank_records, payroll_year, payroll_month))
        actual_voucher_rows.extend(_build_a2_rows(company, company_code, bank_records, payroll_year, payroll_month))
        actual_voucher_rows.extend(_build_a3_rows(company, company_code, company_ws, bank_records, payroll_year, payroll_month))
        ...
        accrual_voucher_rows.extend(_build_a5_rows(company, company_code, company_ws, total_row_idx, payroll_year, payroll_month))
        accrual_voucher_rows.extend(_build_a6_rows(company, company_code, company_ws, payroll_year, payroll_month, timesheet_context))

        bonus_voucher_rows = []
        bonus_voucher_rows.extend(_build_a7_rows(company, company_code, payroll_year, payroll_month, bonus_context))
        bonus_voucher_rows.extend(_build_a8_rows(company, company_code, payroll_year, payroll_month, bonus_context, timesheet_context))
        co_voucher_rows = []
        if company == '耐数电子' and co_context is not None:
        ...
        rd_voucher_rows = []
        if shared_expense_context is not None:
        ...
        cross_group_issues = _validate_cross_group_accounts(actual_voucher_rows)
        ...
        co_voucher_output_path = ''
        if co_voucher_rows and co_voucher_wb is not None:
        ...
        rd_voucher_output_path = ''
        if rd_voucher_rows and rd_voucher_wb is not None:
...
def fill_first_sheet_ab(input_path, base_dir, mapping_path, bank_dir, log):
    log('正在读取原始工资单…')
    ...
    if not os.path.isfile(bonus_path):
    ...
    bonus_context = _load_bonus_context(bonus_path, cost_center_map)
    log(
    ...
    log('开始核对银行流水…')
    for company, summary in payroll_summary.items():
        ...
        salary_match = _match_salary_combination(salary_tax_bank['salary_records'], summary['salary'].quantize(Decimal('0.01'), rounding=ROUND_HALF_UP))
    ...
    output_paths = _save_company_workbooks(
        wb, base_dir, input_path, payroll_year, payroll_month, company_bank_results, log
    )
    voucher_paths, voucher_validation_summary = _generate_voucher_files(
        base_dir, output_paths, bank_records, payroll_year, payroll_month, log, timesheet_context, bonus_context
    )
...
class MainWindow:
    ...
    def _build_header(self):
        ...
        self._header_sub = tk.Label(
            title_frame,
            text='填充 A/B 列、匹配 S/T 列，并生成 A1-A9 凭证',
            font=('微软雅黑', 9),
            fg=COLOR_TEXT_SUB,
            bg=COLOR_HEADER_BG,
        )
    ...
    def _show_guide(self):
        ...
        self._clear_body()
        self._clear_footer()
        self._set_header('工资单首步处理工具', '运行前请确认工资单、Mapping 表、工时、奖金、银行流水和 CO工单分摊数据已准备好')
        ...
        co_path = ''
        if len(raw_files) == 1:
            payroll_period = _extract_payroll_period(raw_files[0])
            if payroll_period:
                co_path = _get_co_workorder_path(self._base_dir, payroll_period[0], payroll_period[1])
        row = tk.Frame(content, bg=COLOR_BG, pady=4)
        row.pack(fill='x')
        tk.Label(row, text=' 必须 ', font=('微软雅黑', 8, 'bold'), fg=COLOR_DANGER, bg='#3A1A1A', padx=4).pack(
            side='left', padx=(0, 8)
        )
        tk.Label(
            row,
            text=os.path.join('原始数据', '工资单', RAW_FILE_PATTERN),
            font=('微软雅黑', 10, 'bold'),
            fg=COLOR_TEXT_MAIN,
            bg=COLOR_BG,
            width=40,
        ...
        status_text = '✓ 已找到' if len(raw_files) == 1 else ('✗ 未找到' if not raw_files else '✗ 找到多个')
        status_fg = '#000000' if len(raw_files) == 1 else '#FFFFFF'
        status_bg = COLOR_SUCCESS if len(raw_files) == 1 else COLOR_DANGER
        tk.Label(row, text=f' {status_text} ', font=('微软雅黑', 8, 'bold'), fg=status_fg, bg=status_bg, padx=4).pack(
            side='left', padx=(0, 10)
        )
        tk.Label(
            row,
            text='每月 HR 提供的原始工资单，后续会校验金额并匹配 S/T 列',
            font=('微软雅黑', 9),
            fg=COLOR_TEXT_SUB,
            bg=COLOR_BG,
            anchor='w',
        ...
        mapping_row = tk.Frame(content, bg=COLOR_BG, pady=4)
        ...
        map_status_text = '✓ 已找到' if mapping_exists else '✗ 未找到'
        map_status_fg = '#000000' if mapping_exists else '#FFFFFF'
        map_status_bg = COLOR_SUCCESS if mapping_exists else COLOR_DANGER
        tk.Label(
            mapping_row,
            text=f' {map_status_text} ',
            font=('微软雅黑', 8, 'bold'),
            fg=map_status_fg,
            bg=map_status_bg,
            padx=4,
        ...
        timesheet_row = tk.Frame(content, bg=COLOR_BG, pady=4)
        timesheet_row.pack(fill='x')
        tk.Label(
            timesheet_row,
            text=' 必须 ',
            font=('微软雅黑', 8, 'bold'),
        ...
        tk.Label(
            timesheet_row,
            text=os.path.join('原始数据', '工时数据', '工时数据.xlsx'),
            font=('微软雅黑', 10, 'bold'),
            fg=COLOR_TEXT_MAIN,
            bg=COLOR_BG,
            width=40,
        ...
        timesheet_status_text = '✓ 已找到' if timesheet_exists else '✗ 未找到'
        timesheet_status_fg = '#000000' if timesheet_exists else '#FFFFFF'
        timesheet_status_bg = COLOR_SUCCESS if timesheet_exists else COLOR_DANGER
        tk.Label(
            timesheet_row,
            text=f' {timesheet_status_text} ',
            font=('微软雅黑', 8, 'bold'),
            fg=timesheet_status_fg,
            bg=timesheet_status_bg,
            padx=4,
        ).pack(side='left', padx=(0, 10))
        tk.Label(
            timesheet_row,
            text='内部订单来源表，运行前会先确保所有项目都能成功匹配',
            font=('微软雅黑', 9),
            fg=COLOR_TEXT_SUB,
            bg=COLOR_BG,
            anchor='w',
        ...
        bonus_row = tk.Frame(content, bg=COLOR_BG, pady=4)
        ...
        tk.Label(
            bonus_row,
            text=os.path.join('原始数据', '奖金数据', '年终奖计提2026_ - to财务.xlsx'),
            font=('微软雅黑', 10, 'bold'),
            fg=COLOR_TEXT_MAIN,
            bg=COLOR_BG,
            width=40,
        ...
        bonus_status_text = '✓ 已找到' if bonus_exists else '✗ 未找到'
        bonus_status_fg = '#000000' if bonus_exists else '#FFFFFF'
        bonus_status_bg = COLOR_SUCCESS if bonus_exists else COLOR_DANGER
        tk.Label(
            bonus_row,
            text=f' {bonus_status_text} ',
            font=('微软雅黑', 8, 'bold'),
            fg=bonus_status_fg,
            bg=bonus_status_bg,
            padx=4,
        ...
        tk.Label(
            bonus_row,
            text='年终奖数据来源表，用于生成 A7/A8 年终奖凭证',
            font=('微软雅黑', 9),
            fg=COLOR_TEXT_SUB,
            bg=COLOR_BG,
            anchor='w',
        ...
        co_row = tk.Frame(content, bg=COLOR_BG, pady=4)
        co_row.pack(fill='x')
        tk.Label(
            co_row,
            text=' 可选 ',
        ...
        tk.Label(
            co_row,
            text='耐数电子/处理月份/CO工单分摊/CO工单分摊.xlsx',
            font=('微软雅黑', 10, 'bold'),
            fg=COLOR_TEXT_MAIN,
            bg=COLOR_BG,
            width=40,
        ...
        co_status_text = '✓ 已找到' if co_exists else '· 未提供'
        co_status_fg = '#000000' if co_exists else '#FFFFFF'
        co_status_bg = COLOR_SUCCESS if co_exists else '#666666'
        tk.Label(
            co_row,
            text=f' {co_status_text} ',
            font=('微软雅黑', 8, 'bold'),
            fg=co_status_fg,
            bg=co_status_bg,
            padx=4,
        ...
        tk.Label(
            co_row,
            text='仅耐数电子使用；若存在则额外生成 A9 CO工单分摊凭证',
            font=('微软雅黑', 9),
            fg=COLOR_TEXT_SUB,
            bg=COLOR_BG,
            anchor='w',
        ...
        bank_files = _find_bank_files(self._bank_dir)
        bank_row = tk.Frame(content, bg=COLOR_BG, pady=4)
        ...
        tk.Label(
            bank_row,
            text=os.path.join('原始数据', '银行流水', BANK_FILE_PATTERN),
            font=('微软雅黑', 10, 'bold'),
            fg=COLOR_TEXT_MAIN,
            bg=COLOR_BG,
            width=40,
        ...
        bank_status_text = '✓ 已找到' if bank_exists else '✗ 未找到'
        bank_status_fg = '#000000' if bank_exists else '#FFFFFF'
        bank_status_bg = COLOR_SUCCESS if bank_exists else COLOR_DANGER
        tk.Label(
            bank_row,
            text=f' {bank_status_text} ',
            font=('微软雅黑', 8, 'bold'),
            fg=bank_status_fg,
            bg=bank_status_bg,
            padx=4,
        ...
        tk.Label(
            bank_row,
            text='自动识别以“银行流水”开头的 Excel，用于工资/公积金/个税/社保核对',
            font=('微软雅黑', 9),
            fg=COLOR_TEXT_SUB,
            bg=COLOR_BG,
            anchor='w',
        ...
        can_run = len(raw_files) == 1 and mapping_exists and timesheet_exists and bonus_exists and bank_exists
        if can_run:
            ...
            tk.Label(
                self._footer,
                text='请确保工资单目录下只有 1 个原始工资单，且工作区域下存在 Mapping表.xlsx、工时数据.xlsx、年终奖计提表 和 银行流水 文件',
                font=('微软雅黑', 9, 'bold'),
                fg=COLOR_DANGER,
                bg=COLOR_BG,
    ...
    def _show_result(self, success, summary):
        ...
        if success and self._result:
            ...
            for company, validation in voucher_validation.items():
                a4_status = '通过' if validation.get('a4_balanced') else '异常'
                a5_status = '通过' if validation.get('a5_balanced') else '异常'
                a6_status = '通过' if validation.get('a6_balanced') else '异常'
                a7_status = '通过' if validation.get('a7_balanced') else '异常'
                a8_status = '通过' if validation.get('a8_balanced') else '异常'
                cross_group_issues = validation.get('cross_group_issues', [])
                if cross_group_issues:
                    voucher_validation_lines.append(
                        f'{company}：A4平衡={a4_status}；A5平衡={a5_status}；A6平衡={a6_status}；A7平衡={a7_status}；A8平衡={a8_status}；A1-A4科目对冲异常 {len(cross_group_issues)} 项'
                    )
                    for issue in cross_group_issues:
                        voucher_validation_lines.append(f'  - {issue}')
                else:
                    voucher_validation_lines.append(
                        f'{company}：A4平衡={a4_status}；A5平衡={a5_status}；A6平衡={a6_status}；A7平衡={a7_status}；A8平衡={a8_status}；A1-A4科目对冲通过'
                    )
    ...
    def _start_run(self):
        raw_files = _find_raw_files(self._raw_dir)
        if len(raw_files) != 1:
            messagebox.showerror('无法运行', '请确保 工资单 目录下只有 1 个符合命名规则的原始工资单文件。')
            return
        if not os.path.exists(self._mapping_path):
            messagebox.showerror('无法运行', '找不到 Mapping表.xlsx，请先放在工作区域目录下。')
            return
        if not os.path.exists(self._timesheet_path):
            messagebox.showerror('无法运行', '找不到 工时数据.xlsx，请先放入 原始数据/工时数据 目录。')
            return
        if not os.path.exists(self._bonus_path):
            messagebox.showerror('无法运行', '找不到 年终奖计提文件，请先放入 原始数据/奖金数据 目录。')
            return
        if not _find_bank_files(self._bank_dir):
            messagebox.showerror('无法运行', '找不到银行流水文件，请先放入 原始数据/银行流水 目录。')
            return
        ...
        def run():
            try:
                ...
                self._q.put(('step', 4, '核对银行流水'))
                result = fill_first_sheet_ab(input_path, self._base_dir, self._mapping_path, self._bank_dir, log)

                self._q.put(('step', 5, '校验输出文件'))
                log('公司拆分文件写出完成', 'ok')
                self._q.put(('step', 6, '完成'))
                summary = '处理完成，所有校验、匹配、银行核对与凭证校验均通过'
